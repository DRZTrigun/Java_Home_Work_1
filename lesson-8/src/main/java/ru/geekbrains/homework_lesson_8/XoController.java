...
public class XoController {
    ...
    private int playerOneDot;                   //символ первого игрока
    private int playerTwoDot;                   // символ второго игрока

    private XoFieldPanel fieldPanel;

    private static final Random RANDOM = new Random();

    //связываем сонтроллер с панелью для отрисовки хода игрока и АИ
    public XoController(XoFieldPanel fieldPanel) {
        this.fieldPanel = fieldPanel;
    }
    //делаем гетор для передачи поля после заполнения в панель, отдаем ссылку на наше поле
    public int[][] getField() {
        return field;
    }
    //гетор на провреку что игра активна
    public boolean isGameActive() {
        return gameActive;
    }
    // гетор для получения состояния
    ...
    public void initialize(int gameMode, int fieldSize, int winLength) {
        ...
        this.playerOneDot = XoConstants.DOT_X;        //символ первого игрока X
        this.playerTwoDot = XoConstants.DOT_O;        // символ второго игрока 0
    ...
    public void processMove(int cellX, int cellY) {
        ...
        field[cellY][cellX] = playerOneDot;        //ставим символ игрока, которая обработала панелька
        fieldPanel.repaint();                      // перисовка панели
        ...
        aiTurn(playerTwoDot, playerOneDot);          // ход АИ
        fieldPanel.repaint();                        // перисовка панели
        // проверка победы АИ
